...
if(isset($_POST["giris"])){
  ...
  if(isset($email)&&isset($parola)){
    ...
    $secim="select * from Musteriler where Eposta ='$email'";//email unıque tanımlı yanı bir tane var
    ...
    if($calistir->rowCount()){
       ...
       // print_r($sifre);
        ...
        if($parola==$sifre){//parola ile sifre uyusuyormu
            ...
            $_SESSION["email"]=$ilgilikayit[0]["Eposta"];//kaydin emailini sessiona attik.
            $_SESSION["name"]=$ilgilikayit[0]["MusteriAd"];//bu degerleri veritabnindan cektigimiz degerden  aliyoruz.
            $_SESSION["surname"]=$ilgilikayit[0]["MusteriSoyad"];
            $_SESSION["MusteriId"]=$ilgilikayit[0]["MusteriId"];
            $_SESSION["adres"]=$ilgilikayit[0]["Adres"];
